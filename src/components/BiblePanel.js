...
export function createBiblePanel(container) {
  ...
  const performSearch = async () => {
    ...
    // Parse "Book Chap:Verse" (very basic parsing)
    const match = query.match(/([a-zA-Z\s]+)\s+(\d+):(\d+)/)
    if (!match) {
      resultsDiv.innerHTML = '<div style="color: #d13438;">Invalid format. Use "Book Chapter:Verse" (e.g. John 3:16)</div>'
      return
    }
    ...
    try {
      const dataModule = await import(`../data/bible-${translation}.json`, { with: { type: 'json' } })
      ...
      if (bibleData[book] && bibleData[book][chap] && bibleData[book][chap][verse]) {
        ...
        const displayRef = `${book} ${chap}:${verse} ${translation.toUpperCase()}`
        ...
        resultsDiv.innerHTML = '<div style="color: #888;">Verse not found in dummy data. (Try Genesis 1:1 or John 3:16)</div>'
      }
    } catch (e) {
      console.error(e)
