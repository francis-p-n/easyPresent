...
export function createSongImporter(onImport) {
  ...
  templateOption.addEventListener(
    'mouseenter',
    () => (templateOption.style.backgroundColor = 'var(--bg-hover)')
  )
  templateOption.addEventListener(
    'mouseleave',
    () => (templateOption.style.backgroundColor = 'transparent')
  )
  ...
  maxLinesContainer.innerHTML =
    '<span style="font-size: 12px; color: var(--text-muted);">Max Lines:</span>'
...
function parseChordProToSlides(text, maxLines = 4) {
  ...
  lines.forEach((line) => {
    ...
    const headerMatch = trimmed.match(
      /^\[?(Verse|Chorus|Bridge|Pre-Chorus|Intro|Outro|Tag)(?:[\s:]+(.+))?\]?:?$/i
    )
